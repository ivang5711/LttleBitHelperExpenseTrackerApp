    ...
    internal class Program
    {
        ...
        public static async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            if (update.Type == UpdateType.Message)
            {
                    ...
                    if (check)
                    {
                        ...
                        if (messageRecieved.Length >= 4 && messageRecieved[0..4] == "/new")
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    float amountParsed;
                                    if (float.TryParse(messages[2], out amountParsed))
                                    {
                                        Console.WriteLine("Parsed successfully!");
                                    }
                                    else
                                    {
                                        await botClient.SendTextMessageAsync(message.Chat, "Parsing failure");
                                        return;
                                    }
                                    ...
                                    Console.WriteLine($"database path: {dbPath}.");
                                    Thread.Sleep(100);
                                    sql = $"INSERT INTO expenses (expenseType, expenseAmount, expenseComment, dateTime, userId, currency) VALUES ('{messages[1]}', {amountParsed}, '{parsedComment}', '{DateTime.UtcNow.ToString()}', {message.Chat.Id}, '{defaultCurrency}');";
                                    Thread.Sleep(100);
                                    ...
                                    if (res > 0)
                                    {
                                        await botClient.SendTextMessageAsync(message.Chat, $"New record added successfully");
                                    }
                                    else
                                    {
                                        await botClient.SendTextMessageAsync(message.Chat, $"The record was not added");
                                    }

                                }
                                else
                                {
                                    await botClient.SendTextMessageAsync(message.Chat, "Wrong arguments. Try again...");
                                }
                            }
                        }
                        ...
                        if (messageRecieved.Length >= 4 && messageRecieved[0..4] == "/all")
                        {
                            ...
                            {
                                await botClient.SendTextMessageAsync(message.Chat, "No records so far...");
                            }
                        ...
                        if (messageRecieved.Length >= 6 && messageRecieved[0..6] == "/typed")
                        {
                            ...
                            if (UsersList.NList.Count > 0)
                            {
                                ...
                                string resString = $"Expense summary calculated in your default currency: {defaultCurrency}";
                                foreach (var item in UsersList.FinalList)
                                {
                                    resString += $"\nTYPE: {item.ExpenseType} | AMMOUNT: {item.ExpenseAmount}";
                                }

                                await botClient.SendTextMessageAsync(message.Chat, resString);
                            }
                            else
                            {
                                await botClient.SendTextMessageAsync(message.Chat, "No records so far...");
                        ...
                        if (messageRecieved.Length >= 8 && messageRecieved[0..8] == "/history")
                        {
                            ...
                            if (result.Count > 0)
                            {
                                string resString = "Prints out all the records history";
                                foreach (var item in result)
                                {
                        ...
                        if (messageRecieved.Length >= 7 && messageRecieved[0..7] == "/delete")
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    int delId;
                                    if (int.TryParse(messages[1], out delId))
                                    {
                                        Console.WriteLine("Parsed successfully!");
                                    }
                                    ...
                                    if (result.Count > 0)
                                    {
                                        ...
                                        if (res > 0)
                                        {
                                            await botClient.SendTextMessageAsync(message.Chat, $"Record with ID {delId} deleted successfully");
                                        }
                                        else
                                        {
                                            await botClient.SendTextMessageAsync(message.Chat, $"failed to delete record with ID {delId}");
                                        }
                                    }
                                    else
                                    {
                                        await botClient.SendTextMessageAsync(message.Chat, "no such id in your records");
                                    }
                                }
                                else
                                {
                                    await botClient.SendTextMessageAsync(message.Chat, "Wrong argument. Try again...");
                                }
                            }
                        }
                        ...
                        if (messageRecieved.Length >= 9 && messageRecieved[0..9] == "/currency")
                        {
                            if (messageRecieved.TrimEnd() == "/currency")
                            {
                                ...
                                foreach (var item in result)
                                {
                                    Console.WriteLine(item.LocalCurrency);
                            ...
                            else
                            {
                                ...
                                if (messages.Count == 2)
                                {
                                    if (CurrencyKeys.Contains(messages[1].ToUpper()))
                                    {
                                        ...
                                        if (result > 0)
                                        {
                                            await botClient.SendTextMessageAsync(message.Chat, $"Your default currency updated");
                                        }
                                        else
                                        {
                                            await botClient.SendTextMessageAsync(message.Chat, $"Your default currency was not updated");
                                        }
                                    }
                                    else
                                    {
                                        await botClient.SendTextMessageAsync(message.Chat, $"{messages[1].ToUpper()} not found...");
                                    }
                                }
                                else
                                {
                ...
                {
                    await botClient.SendTextMessageAsync(message.Chat, "Command not recognized. Try /help command");
                }
            }
        }
